...
- (CGPoint)contentOffsetForCellState:(SWCellState)state
{
    ...
    switch (state)
    {
        ...
        case kCellStateRight:
            ...
            //[self leftUtilityButtonsWidth] + [[UIScreen mainScreen] bounds].size.width;
            break;
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    ...
    else if (velocity.x <= -0.5f)
    {
        ...
        if (!self.leftUtilityButtons || self.leftUtilityButtonsWidth == 0.0)
        {
            _cellState = kCellStateCenter;
        }
        else
    ...
    if (scrollView.contentOffset.x > [self leftUtilityButtonsWidth])
    {
        if ([self rightUtilityButtonsWidth] > 0)
        {
            if (self.delegate && [self.delegate respondsToSelector:@selector(swipeableTableViewCell:canSwipeToState:)])
            {
                ...
                if (!shouldScroll)
                {
                    //Added [scrollView setContentOffset:CGPointMake([self leftUtilityButtonsWidth], 0)];
